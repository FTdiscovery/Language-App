...
class BasicGerman: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "mainGerman" {
            let splash:GermanViewController = segue.destination as! GermanViewController
            splash.username = username
            splash.password = password
        }
        if segue.identifier == "courseTable" {
            let navController:UINavigationController = segue.destination as! UINavigationController
            let table:GermanView = navController.topViewController as! GermanView
