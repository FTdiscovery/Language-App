...
class RalyanRewardPage: UIViewController {
        ...
        FIRAuth.auth()?.signIn(withEmail: username, password: password, completion: {(user,error) in
            ...
            if !(error != nil) {
                ...
                findScore.observe(.value) { (snap: FIRDataSnapshot) in
                    ...
                    if (score!>=10) {
                        self.performSegue(withIdentifier: "rf1", sender: nil)
                    }
                    else {
        ...
        FIRAuth.auth()?.signIn(withEmail: username, password: password, completion: {(user,error) in
            ...
            if !(error != nil) {
                ...
                findScore.observe(.value) { (snap: FIRDataSnapshot) in
                    let score = Int((snap.value as AnyObject).description)
                    if (score!>=50) {
                        self.performSegue(withIdentifier: "rf2", sender: nil)
                    }
                    else {
                        let alert = UIAlertController(title: "Uh oh.", message: "Not enough Points.", preferredStyle: UIAlertControllerStyle.alert)
                        alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.cancel, handler: nil))
        ...
        FIRAuth.auth()?.signIn(withEmail: username, password: password, completion: {(user,error) in
            ...
            if !(error != nil) {
                ...
                findScore.observe(.value) { (snap: FIRDataSnapshot) in
                    let score = Int((snap.value as AnyObject).description)
                    if (score!>=100) {
                        self.performSegue(withIdentifier: "rf3", sender: nil)
                    }
                    else {
                        let alert = UIAlertController(title: "Uh oh.", message: "Not enough Points.", preferredStyle: UIAlertControllerStyle.alert)
                        alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.cancel, handler: nil))
        ...
        FIRAuth.auth()?.signIn(withEmail: username, password: password, completion: {(user,error) in
            ...
            if !(error != nil) {
                ...
                findScore.observe(.value) { (snap: FIRDataSnapshot) in
                    let score = Int((snap.value as AnyObject).description)
                    if (score!>=250) {
                        self.performSegue(withIdentifier: "rf4", sender: nil)
                    }
                    else {
                        let alert = UIAlertController(title: "Uh oh.", message: "Not enough Points.", preferredStyle: UIAlertControllerStyle.alert)
                        alert.addAction(UIAlertAction(title: "OK", style: UIAlertActionStyle.cancel, handler: nil))
        ...
        FIRAuth.auth()?.signIn(withEmail: username, password: password, completion: {(user,error) in
            ...
            if !(error != nil) {
                ...
                findScore.observe(.value) { (snap: FIRDataSnapshot) in
                    let score = Int((snap.value as AnyObject).description)
                    if (score!>=500) {
                        self.performSegue(withIdentifier: "rf5", sender: nil)
                    }
                }
            }
            else {
